...
import './index.css';
import { PyJetsSeatMapService } from './pyService';

export const usePYSeatMapRef = () => useRef(null);
export const PYSeatMap = forwardRef(
  (
    {
      flight,
      availability,
    ...
    const colorTheme = JetsDataHelper.mergeColorThemeWithConstraints(
      PYSeatMap.defaultProps.config.colorTheme,
      config.colorTheme
    );
    config.colorTheme = colorTheme;
    config.lang = JetsDataHelper.validateLanguage(config.lang);
    const configuration = { ...PYSeatMap.defaultProps.config, ...config };
    ...
    const service = new PyJetsSeatMapService(configuration);
    ...
    useEffect(() => {
      ...
      if (flight?.id) {
        service
          .getSeatMapData(flight, availability, passengers, configuration, includedCabins)
          .then(data => {
